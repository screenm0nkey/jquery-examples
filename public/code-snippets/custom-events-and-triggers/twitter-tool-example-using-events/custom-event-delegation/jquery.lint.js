...
(function(){
    ...
    emptyFn = function(){
        ...
        warn: glob.console && console.warn ? function(){
            console.warn.apply(console, arguments);
        }
 : emptyFn,
        
        group: glob.console && console.group ? function(){
            console.group.apply(console, arguments);
        }
 : emptyFn,
        
        groupEnd: glob.console && console.groupEnd ? function(){
            console.groupEnd();
        }
 : emptyFn,
        
        groupCollapsed: glob.console && console.groupCollapsed ? function(){
            console.groupCollapsed.apply(console, arguments);
        }
 : emptyFn,
        
        log: glob.console && console.log ? function(){
 ...
 : emptyFn
    ...
    checks = [{/* Level 0 */}, {/* Level 1 */}, {/* Level 2 */}, {/* Level 3 */}], addCheck = function(methodName, level, check){
    ...
    }, api = lint.api, // Only cover certain fns under the jQ namespace
 coveredNamespace = /^(getJSON|extend|ajax|get|post|proxy|each|map|queue|ajax|ajaxSetup|removeData|data|pushStack)$/, version = _jQuery.fn.jquery, map = _jQuery.map, each = _jQuery.each, extend = _jQuery.extend, find = _jQuery.find, undefined, arrSlice = Array.prototype.slice, slice = function(a, s, e){
    ...
    }, toString = Object.prototype.toString, typeToString = function(o){
 ...
 types = {
    ...
    }, selectorCache = {}, jQueryMethods = extend({}, _jQuery.fn), internal = false, fromInit = false;
    ...
    function logLocation(){
        ...
        try {
            ...
            if (e.stack) {
                ...
                lint.console.log(e.stack // Remove everything before the file name and line number
...
.replace(/(^|\n)(.+?)\n(?=\2(?:\n|$)|[\s\S]+?\n\2(?:\n|$))/g, '$1'));
    ...
    function isValidArgumentList(args, sig){
        ...
        var matches = false, sigArg, argLength = args.length, nextIsOptional = false;
        ...
        for (var sigIndex = 0, argIndex = 0, fullLength = Math.max(argLength, sig.arg.length || 1); sigIndex < fullLength || argIndex < argLength; ++sigIndex) {
    ...
    function isValidType(type, arg){
        ...
        var split = type.split(/,\s?/g), i = split.length, cur;
    ...
    function registerMethod(name, methodAPI){
        ...
        var obj = /^jQuery\./.test(name) ? _jQuery : _jQuery.fn, methodName = name.replace(/^jQuery\./, '');
        
        obj[methodName] = (function(meth, name){
            return extend(function(){
            
                var args = slice(arguments), _internal = internal;
    ...
    function coverMethod(name, meth, args){
        ...
        var locale = lint.langs[lint.lang], sigs = api[name], _console = lint.console, self = this, i = 0, sig, specialCheckResults = (function(){
        
            // Perform special checks for current level and
            // all levels below current level.
            
            var lvl = lint.level + 1, results = [], check;
            
            while (lvl--) {
                if (checks[lvl] && (check = checks[lvl][name])) {
                    if (types.array(check)) {
                        each(check, function(i, chk){
                            results.push(chk.apply(self, args));
                        })
                    }
                    else {
                        results.push(check.apply(self, args));
                    }
                }
            }
        ...
        }()), signatureMatch = false, sliced = slice(this, 0, 10);
        ...
        if (!signatureMatch) {
            ...
            each(sigs, function(i, sig){
                ...
                var sigArgs = sig.arg;
                _console.log(name + '(' +
                (sigArgs ? sigArgs[0] ? map(sigArgs, function(sig, i){
        ...
        try {
            ...
            _console.warn(locale.errorThrown.replace(/%0/, name), e);
    ...
    _jQuery.fn.init = (function(_init){
        ...
        return function(selector, context){
            ...
            var locale = lint.langs[lint.lang], ret = coverMethod.call(this, 'jQuery', function(){
            
                return runFunction(function(){
                    return new _init(selector, context);
                }, [], true, this)
                
            }, arguments), _console = lint.console;
    ...
    each(['css', 'attr', 'bind', 'one'], function(i, methodName){
        ...
        addCheck(methodName, 3, function(){
            ...
            var args = arguments, hoc = this, locale = lint.langs[lint.lang], sliced = slice(hoc, 0, 10), _console = lint.console;
            ...
            (/^(bind|one)$/.test(methodName) && version >= '1.4' && /* Data no passed as [1] */ !isFunction(args[2])))) {
                ...
                if (this._lastMethodCalled === methodName) {
                    ...
                    _console.log(locale.collection, sliced);
                    _console.log(args);
                    _console.log(locale.combineCalls.replace(/%0/, methodName).replace(/%1/, '{\n' +
    ...
    each(['find', 'children', 'parent', 'parents', 'next', 'nextAll', 'prev', 'prevAll', 'first', 'last', 'closest', 'siblings', 'parentsUntil', 'nextUntil', 'prevUntil'], function(i, methodName){
